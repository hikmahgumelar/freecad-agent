...
DEFAULT_SNAPFIT_OUTPUT = PROJECT_ROOT / "cad" / "output" / "esp32-c3-super-mini-snapfit-v1.FCStd"


def resolve_project_path(value, default_path):
    """Resolve CAD output paths relative to this checkout, independent of OS/user."""
    if not value:
    ...
    # Normalize legacy absolute paths such as /home/hikmah/projectx/freecad-agent/...
    # or /Users/hikmah/projectx/freecad-agent/... to this checkout.
    marker = "/freecad-agent/"
    raw = str(path)
    if marker in raw:
...
class FreeCADExecutor:
    ...
    def execute(self, job):
        ...
        if action == "open_model":
            p = job.get("parameters", {})
            path = resolve_project_path(p.get("path"), PROJECT_ROOT)
        ...
        if action == "create_case_rails":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "case-v1-with-rails-v2.FCStd")
            return self._send({"action": "create_case_rails", "body": p.get("body", "Body"), "rail_width": p.get("rail_width", 1.6), "rail_height": p.get("rail_height", 1.8), "inset": p.get("inset", 2.0), "clearance": p.get("clearance", 0.2), "output_path": output})
        if action == "create_slider_cover":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "case-v1-with-slider-v1.FCStd")
            return self._send({"action": "create_slider_cover", "body": p.get("body", "Body"), "rail_width": p.get("rail_width", 1.6), "rail_height": p.get("rail_height", 1.8), "rail_inset": p.get("rail_inset", 2.0), "clearance": p.get("clearance", 0.2), "top_clearance": p.get("top_clearance", 0.2), "thickness": p.get("thickness", 1.6), "antenna_offset": p.get("antenna_offset", 10.0), "end_clearance": p.get("end_clearance", 4.0), "skirt_width": p.get("skirt_width", 1.6), "output_path": output})
        if action == "create_box_enclosure":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "box-90x60x11.FCStd")
            return self._send({"action": "create_box_enclosure", "length": p.get("length", 90.0), "width": p.get("width", 60.0), "height": p.get("height", 11.0), "wall": p.get("wall", 1.5), "cover_thickness": p.get("cover_thickness", 1.5), "screw_diameter": p.get("screw_diameter", 3.0), "screw_margin": p.get("screw_margin", 5.0), "antenna_diameter": p.get("antenna_diameter", 7.0), "antenna_left_offset": p.get("antenna_left_offset", 10.0), "usb_width": p.get("usb_width", 12.0), "usb_height": p.get("usb_height", 5.0), "post_outer_diameter": p.get("post_outer_diameter", 6.0), "output_path": output})
        if action == "create_large_enclosure":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "enclosure-200x100x250.FCStd")
            return self._send({"action": "create_large_enclosure", "length": p.get("length", 200.0), "width": p.get("width", 100.0), "height": p.get("height", 250.0), "wall": p.get("wall", 2.0), "bottom_thickness": p.get("bottom_thickness", 2.0), "lid_length": p.get("lid_length", 202.0), "lid_width": p.get("lid_width", 102.0), "lid_height": p.get("lid_height", 60.0), "lid_wall": p.get("lid_wall", 2.0), "plate_center_z": p.get("plate_center_z", 125.0), "plate_thickness": p.get("plate_thickness", 2.0), "hole_diameter": p.get("hole_diameter", 50.0), "hole_columns": p.get("hole_columns", 3), "hole_rows": p.get("hole_rows", 2), "output_path": output})
        ...
        if action == "create_medicine_cover":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "medicine-box-cover-v1.FCStd")
            return self._send({"action": "create_medicine_cover", "output_path": output})
        if action == "create_medicine_plate":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "medicine-box-plate-6holes-v1.FCStd")
            return self._send({"action": "create_medicine_plate", "output_path": output})
        if action == "create_medicine_body":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "medicine-box-body-v1.FCStd")
            return self._send({"action": "create_medicine_body", "output_path": output})
        if action == "create_smoke_test_box":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "smoke-test-box-v1.FCStd")
            return self._send({
        ...
        if action == "create_character_figurine":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), PROJECT_ROOT / "cad" / "output" / "character-figurine-120mm.FCStd")
            stl = resolve_project_path(p.get("stl_path"), PROJECT_ROOT / "cad" / "output" / "character-figurine-120mm.stl")
            return self._send({"action": "create_character_figurine", "height": p.get("height", 120.0), "base_diameter": p.get("base_diameter", 46.0), "base_thickness": p.get("base_thickness", 3.0), "output_path": output, "stl_path": stl})
        if action == "create_snapfit_case":
            p = job.get("parameters", {})
            output = resolve_project_path(p.get("output_path"), DEFAULT_SNAPFIT_OUTPUT)
            command = dict(p)
            command["action"] = "create_snapfit_case"
