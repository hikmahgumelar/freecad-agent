...
def _inspect_geometry(command):
    ...
    result = {
        ...
        "bounding_box": {
            ...
            "x_length": bb.XLength, "y_length": bb.YLength, "z_length": bb.ZLength,
        },
...
def _create_slider_cover(command):
    ...
    rail_left = Part.makeBox(width, rail_width, rail_height, App.Vector(x0, y0 + rail_width, rail_z))
    rail_right = Part.makeBox(width, rail_width, rail_height, App.Vector(x0, y0 + depth - 2 * rail_width, rail_z))
    ...
    return {
        ...
        "dimensions": {"width": width, "depth": depth, "thickness": thickness,
                       "rail_width": rail_width, "rail_height": rail_height, "clearance": clearance},
...
def execute_command(command):
    ...
    if action == "create_sphere":
        ...
        doc.recompute()
        _fit_view(doc)
        return {"ok": True, "action": "create_sphere", "object": sphere.Name,
                "radius": radius, "document": doc.Name}
    ...
    if action == "open_model":
        ...
        _fit_view(doc)
        return {"ok": True, "action": "open_model", "path": path,
                "document": doc.Name, "label": doc.Label}
    ...
    if action == "inspect_model":
        ...
        return {"ok": True, "action": "inspect_model", "document": doc.Name,
                "label": doc.Label, "objects": objects}
    ...
    if action == "inspect_features":
        ...
        for name in names:
            ...
            if obj is None:
                features.append({"name": name, "error": "Object not found"})
                continue
            features.append(_inspect_feature(obj))
        return {"ok": True, "action": "inspect_features", "document": doc.Name,
                "features": features}
