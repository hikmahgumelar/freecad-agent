...
def _centers(size, count, diameter, margin):
    ...
    if available < required:
        raise RuntimeError(
            f"Cannot fit {count} x Ø{diameter:.1f} mm holes in {size:.1f} mm with {margin:.1f} mm margin"
        )
    if count == 1:
        return [size / 2.0]
    start = margin + radius
...
def create_medicine_box(command):
    """Create Medicine Box v1 as exactly three independent printable solids."""
    ...
    output_path = os.path.abspath(command.get(
        "output_path",
        "cad/output/medicine-box-v1.FCStd"
    ))

    if min(L, W, H, wall, bottom, plate_t, ledge_t, ledge_width, hole_d,
           plate_clearance, cover_clearance, cover_wall, cover_top, cover_insertion) <= 0:
    ...
    # BODY: hollow box, plus a continuous 1 mm thick internal ledge on all four sides.
    ...
    ledge_outer = Part.makeBox(
        inner_L,
        inner_W,
        ledge_t,
        App.Vector(wall, wall, plate_z - ledge_t)
    )
    ledge_inner = Part.makeBox(
        max(0.01, inner_L - 2.0 * ledge_width),
        max(0.01, inner_W - 2.0 * ledge_width),
        ledge_t + 0.2,
        App.Vector(wall + ledge_width, wall + ledge_width, plate_z - ledge_t - 0.1)
    )
    ledge_ring = ledge_outer.cut(ledge_inner)
    body_shape = body_shape.fuse(ledge_ring)

    body = doc.addObject("Part::Feature", "MedicineBody")
    body.Label = f"Medicine Box Body {L:.0f}x{W:.0f}x{H:.0f} mm"
    body.Shape = body_shape

    # PLATE: deliberately smaller than the body cavity by 0.2 mm per side.
    ...
    # COVER: outer cap. Body enters the cover by exactly 60 mm.
    ...
    cover_outer = Part.makeBox(
        cover_outer_L, cover_outer_W, cover_H,
        App.Vector(cover_x, cover_y, cover_z)
    )
    cover_cavity = Part.makeBox(
        cover_inner_L, cover_inner_W, cover_insertion,
        App.Vector(cover_x + cover_wall, cover_y + cover_wall, cover_z)
    )
    ...
    # Metadata for inspection and downstream validation.
    for obj, props in (
        (body, (("Length", L), ("Width", W), ("Height", H),
                ("WallThickness", wall), ("BottomThickness", bottom),
                ("PlateLedgeZ", plate_z), ("PlateLedgeThickness", ledge_t),
                ("PlateLedgeWidth", ledge_width))),
        (plate, (("Length", plate_L), ("Width", plate_W), ("Thickness", plate_t),
                  ("BottomZ", plate_z), ("ClearancePerSide", plate_clearance),
                  ("HoleDiameter", hole_d), ("HoleEdgeMargin", hole_margin))),
        (cover, (("Length", cover_outer_L), ("Width", cover_outer_W),
                  ("Height", cover_H), ("WallThickness", cover_wall),
                  ("TopThickness", cover_top), ("InsertionDepth", cover_insertion),
                  ("ClearancePerSide", cover_clearance))),
    ):
    ...
    plate.HoleCenters = (
        "X=" + ", ".join(f"{v:.2f}" for v in x_centers) +
        " mm; Y=" + ", ".join(f"{v:.2f}" for v in y_centers) + " mm"
    )
    ...
    # Printability checks: each part is an independent solid and the intended plate/body fit is non-interfering.
    ...
    return {
        ...
        "body": {"length": L, "width": W, "height": H, "wall": wall,
                 "plate_ledge_z": plate_z, "plate_ledge_thickness": ledge_t,
                 "plate_ledge_width": ledge_width},
        "plate": {"length": plate_L, "width": plate_W, "thickness": plate_t,
                   "bottom_z": plate_z, "clearance_per_side": plate_clearance,
                   "hole_diameter": hole_d, "hole_count": 6,
                   "pattern": "3 x 2", "hole_centers_x": x_centers,
                   "hole_centers_y": y_centers},
        "cover": {"length": cover_outer_L, "width": cover_outer_W, "height": cover_H,
                  "wall": cover_wall, "top": cover_top,
                  "insertion_depth": cover_insertion,
                  "clearance_per_side": cover_clearance},
        "printability": {
            "separate_parts": True,
            "plate_body_clearance_validated": True,
            "intended_orientation": "body upright; plate horizontal; cover upright",
            "support_required_by_design": False,
        },
    }
