...
# Load the base listener from this exact repository directory.
...
# Ensure this repository's freecad/ directory wins module resolution.
if _BASE_DIR not in sys.path:
    sys.path.insert(0, _BASE_DIR)


def _create_box_enclosure(command):
    # Resolve and reload at execution time. This prevents a stale function
    # reference from surviving a listener/module reload.
    module = importlib.import_module("box_enclosure")
    module = importlib.reload(module)
    return module.create_box_enclosure(command)
...
def execute_command(command):
    if command.get("action") == "create_box_enclosure":
        return _create_box_enclosure(command)
